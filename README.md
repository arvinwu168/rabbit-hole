...
A native iPad target lives in [`ios/RabbitHole`](ios/RabbitHole). It embeds the same deployed Next.js interface in a persistent WebKit view, preserving the web app's UI, password session, local chats, branching, themes, streaming, and hosted model routes without putting server secrets into the app binary. See [`ios/README.md`](ios/README.md) for simulator, Vercel URL, signing, and archive instructions.
